...
// Guest Schema for check-in/check-out
...
app.get("/api/hotels/:hotelId/guests", authenticateToken, async (req, res) => {
  try {
    const guests = await Guest.find({ hotelId: req.params.hotelId }).sort({
      createdAt: -1,
    });
    res.json(guests);
  } catch (error) {
...
app.post("/api/hotels/:hotelId/guests", authenticateToken, async (req, res) => {
  try {
    ...
    // Create guest record with billing info
    const guest = new Guest({
      ...
      ratePerNight: guestData.ratePerNight,
      totalNights: guestData.totalNights,
      totalAmount: guestData.totalAmount,
      advancePayment: guestData.advancePayment || 0,
      paidAmount: guestData.paidAmount || 0,
      pendingAmount: guestData.pendingAmount,
      specialRequests: guestData.specialRequests,
      status: "checked-in",
    });

    await guest.save();
    console.log("✅ Guest checked in:", guest.name, "Room:", guest.roomNumber);
...
app.put(
  "/api/hotels/:hotelId/guests/:guestId",
  authenticateToken,
  async (req, res) => {
    try {
...
app.delete(
  ...
  async (req, res) => {
    try {
      ...
      const guest = await Guest.findOneAndDelete({ _id: guestId, hotelId });
